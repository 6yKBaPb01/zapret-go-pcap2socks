...
:: ===== NEW: go-pcap2socks and TG WS Proxy status =====
echo.
echo --- Additional Utilities ---

:: Проверка процессов
tasklist /FI "IMAGENAME eq go-pcap2socks.exe" | find /I "go-pcap2socks.exe" > nul
if !errorlevel!==0 (
    call :PrintGreen "go-pcap2socks.exe is RUNNING."
) else (
    call :PrintRed "go-pcap2socks.exe is NOT running."
)

tasklist /FI "IMAGENAME eq TgWsProxy_windows.exe" | find /I "TgWsProxy_windows.exe" > nul
if !errorlevel!==0 (
    call :PrintGreen "TG WS Proxy (TgWsProxy_windows.exe) is RUNNING."
) else (
    call :PrintRed "TG WS Proxy (TgWsProxy_windows.exe) is NOT running."
)
...
:: ===== NEW: npcap driver check =====
sc query "npcap" >nul 2>&1
if !errorlevel! neq 0 (
    call :PrintRed "[X] Npcap driver is not installed. go-pcap2socks will not start without it."
    echo.
    set "INSTALL_NPCAP="
    set /p "INSTALL_NPCAP=Would you like to install Npcap now? (Y/N, default N): "
    if /i "!INSTALL_NPCAP!"=="Y" (
        if exist "%~dp0utils\npcap-1.87.exe" (
            echo Running npcap installer...
            start /wait "" "%~dp0utils\npcap-1.87.exe"
            echo Please follow the installer instructions.
        ) else (
            call :PrintRed "npcap-1.87.exe not found in utils!"
        )
    )
) else (
    call :PrintGreen "Npcap driver check passed"
)
echo:
:: =====================================
...
if /i "%GameFilterMode%"=="all" (
    ...
    set "GameFilterTCP=1-65535"
    set "GameFilterUDP=1-65535"
...
set "hostsFile=%SystemRoot%\System32\drivers\etc\hosts"
set "hostsUrl=https://raw.githubusercontent.com/6yKBaPb01/zapret-go-pcap2socks/refs/heads/main/.service/hosts"
...
if errorlevel 1 (
    ...
    set "PYTHON_INSTALLER=%~dp0utils\python-3.14.4-amd64.exe"
    if exist "!PYTHON_INSTALLER!" (
        echo Installing Python from bundled installer...
        start /wait "" "!PYTHON_INSTALLER!" /quiet InstallAllUsers=1 PrependPath=1
        where python >nul 2>&1
        if errorlevel 1 (
            call :PrintRed "Python installation failed or not added to PATH. Please install manually and rerun."
            pause
            goto menu
        )
        echo Python installed successfully.
    ) else (
        call :PrintRed "Python installer not found at utils\python-3.14.4-amd64.exe"
        pause
        goto menu
    )
)
