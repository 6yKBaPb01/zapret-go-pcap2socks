...
# Source: general (ALT4).bat (auto-converted)
...
--new
--ipset=$LISTS_DIR/ipset-all.txt --ipset-exclude=$LISTS_DIR/ipset-exclude.txt --ipset-exclude=$LISTS_DIR/ipset-exclude-user.txt --dpi-desync=fake,multisplit --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n2 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=1000 --dpi-desync-fake-tls=$BIN_DIR/stun.bin --dpi-desync-fake-tls=$BIN_DIR/tls_clienthello_www_google_com.bin --dpi-desync-fake-http=$BIN_DIR/tls_clienthello_max_ru.bin
--new
