#!/bin/bash
# Strategy: general (Legacy SINGLE FAKE ALT) (1)
# Source: general (Legacy SINGLE FAKE ALT) (1).bat (auto-converted)
...
--new
--filter-tcp=1-65535 --hostlist-domains=discord.media --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls=$BIN_DIR/tls_clienthello_www_google_com.bin
--new
...
--new
--ipset=$LISTS_DIR/ipset-all.txt --ipset-exclude=$LISTS_DIR/ipset-exclude.txt --ipset-exclude=$LISTS_DIR/ipset-exclude-user.txt --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n4 --dpi-desync-fooling=ts --dpi-desync-fake-tls=$BIN_DIR/tls_clienthello_max_ru.bin --dpi-desync-fake-http=$BIN_DIR/tls_clienthello_max_ru.bin
--new
